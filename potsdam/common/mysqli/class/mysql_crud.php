...
class Database{
	...
	public function connect($path_to_root="../"){
        $this->root_path = $path_to_root;
        require $this->root_path . "common/config.php";

		if(!$this->con){
			$this->myconn = new mysqli($host,$username,$password,$dbname);  // mysql_connect() with variables defined at the start of Database class
	...
	public function sql($sql){
		...
		if($query){
            ...
            if ($query == 1)
            {
                $this->result = $this->myconn->insert_id;
                return true;
            }
	...
	private function tableExists($table){
		...
		$tablesInDb = $this->myconn->query('SHOW TABLES FROM '.$dbname.' LIKE "'.$table.'"');
        if($tablesInDb){
